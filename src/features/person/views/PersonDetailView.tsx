      ...
      <div className="space-y-1 text-sm">
        <div className="flex items-center gap-1.5">
          ...
          {credit.voteAverage > 0 && (
            <Badge variant="outline" className="ml-auto h-5 shrink-0 rounded-full px-1.5 text-[10px]">
  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <section className="relative overflow-hidden rounded-lg">
        {heroBackdrop ? (
          <img src={heroBackdrop} alt="" className="absolute inset-0 h-full w-full object-cover opacity-30" />
        ) : (
        ...
        <Button
          variant="ghost"
          className="absolute top-3 left-3 z-20 h-8 rounded-full px-2.5 !bg-transparent text-white/90 hover:text-white"
          onClick={() => navigate(-1)}
        >
        ...
        <div className="relative z-10 flex flex-col gap-4 p-4 sm:flex-row sm:gap-6 sm:p-6 md:p-8">
          <div className="border-border/30 mx-auto size-28 shrink-0 overflow-hidden rounded-full border-2 bg-black/30 sm:mx-0 sm:size-36 md:size-44">
            {profileUrl ? (
              ...
              <div className="text-muted-foreground flex size-full items-center justify-center text-xs">暂无照片</div>
            )}
          </div>
          ...
          <div className="min-w-0 flex-1 space-y-2 text-center sm:text-left">
            ...
            {person.biography && (
              <ScrollableText className="text-sm leading-6 text-white/80 whitespace-pre-line">
                {person.biography}
              </ScrollableText>
            )}
          </div>
      ...
      <section className="space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-3">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Select
              value={sortBy}
              onValueChange={v => setSortBy(v as CreditSortBy)}
            >
